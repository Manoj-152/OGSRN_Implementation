...
def train_plotter(cfg, generator, trainloader, epoch, path):
    batch_tensor = None
    print('Plotting the Generator Train results')
    for n,(optic,sar) in enumerate(trainloader):
        if n == 10: break
        optic = optic.to(cfg['DEVICE'])
        sar = sar.to(cfg['DEVICE'])
...
def test_plotter(cfg, generator, valloader, epoch, path):
    batch_tensor = None
...
def plot_feature_maps(cfg, generator, discriminator, trainloader, epoch, path):
